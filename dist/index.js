...
import { fetchAllPolymarketMarkets, fetchPolymarketMarketsByConditionIds, normalizePolymarket, resetPolymarketFetchStats, getPolymarketFetchStats, } from "./apis/polymarket.js";
import { fetchAllKalshiMarkets, fetchKalshiMarketsForEventTickersWithTargets, normalizeKalshi, resetKalshiFetchStats, getKalshiFetchStats, getKalshiEffectiveRps, } from "./apis/kalshi.js";
...
function printSummary() {
    ...
    if (stats.bestOpportunity) {
        ...
        console.log(`  Best opportunity:    +${p.toFixed(1)}% on "${stats.bestOpportunity.matchedPair.polymarket.title}"`);
    }
...
async function pollCycle() {
    ...
    try {
        ...
        const polyPromise = doDiscovery
            ...
            : (async () => {
                const t = Date.now();
                const conditionIds = Array.from(new Set(matchedPairs.map((p) => p.polymarket.raw.condition_id)));
        ...
        if (polyRaw.length > 0)
            lastPolyFetch = Date.now();
        if (kalshiRaw.length > 0 || config.demoMode)
    ...
    const polyMarkets = polyRaw
        .map(normalizePolymarket)
        .filter((m) => m !== null);
    const kalshiMarkets = kalshiRaw
        .map(normalizeKalshi)
        .filter((m) => m !== null);
    ...
    arbMs = Date.now() - tArb;
    stats.totalOpportunities += opportunities.filter((o) => opportunityDisplayProfitPct(o) >= config.minProfitPct).length;
